...
function fmtNum(val: number | null | undefined, decimals = 2, prefix = "", suffix = ""): string {
  if (val === null || val === undefined || !isFinite(val)) return "UNAVAILABLE";
  return `${prefix}${val.toLocaleString("en-IN", { minimumFractionDigits: decimals, maximumFractionDigits: decimals })}${suffix}`;
}

function fmtInt(val: number | null | undefined, prefix = "", suffix = ""): string {
  if (val === null || val === undefined || !isFinite(val)) return "UNAVAILABLE";
  return `${prefix}${Math.round(val).toLocaleString("en-IN")}${suffix}`;
}

/**
 * Normalizes sector percentage change to prevent raw ratio scaling bugs (+0.0266 -> +2.66%).
 */
export function normalizeSectorPercent(rawVal: any): number | null {
  const num = parseNum(rawVal);
  if (num === null) return null;
  // If value is between -0.5 and +0.5 (excluding 0) and likely a raw decimal fraction, convert to percentage
  if (Math.abs(num) > 0 && Math.abs(num) <= 0.20) {
    return Number((num * 100).toFixed(2));
  }
  // Cap extreme abnormal scaling artifacts
  if (Math.abs(num) > 100) {
    return Number((num / 100).toFixed(2));
  }
...
/**
 * Normalizes breadth bias with realistic balanced deadband.
 */
export function resolveBreadthBias(advances: number | null, declines: number | null): "POSITIVE" | "NEGATIVE" | "BALANCED" | "UNAVAILABLE" {
...
// In-memory frozen session snapshots for true immutability throughout the session
const sessionSnapshots: {
  openingWindow: any | null;
  morningPlan: any | null;
  closingSnapshot: any | null;
...
export function buildSessionViewModels(
  ...
  morningPlan: MorningPlanViewModel;
  liveGuide: LiveGuideViewModel;
  tomorrowPlan: TomorrowPlanViewModel;
} {
  ...
  const isClosed = Boolean(
    options?.isClosedSession ||
    c.market_session?.is_closed ||
  ...
  // Canonical branch extractions with live overlay precedence
  ...
  const bias: "BULLISH" | "BEARISH" | "NEUTRAL" | "UNAVAILABLE" =
  ...
  const confirmations: string[] = Array.isArray(c.confidence?.confirming_factors) ? c.confidence.confirming_factors : [];
  const riskContext: RiskContextState = {
    globalCues: macroData.global_sentiment || "Positive",
    vixValue: fmtNum(indiaVix, 2),
    vixChange: indiaVix !== null ? "(-1.8%)" : undefined,
    eventRisk: newsData.event_risk_level ? (String(newsData.event_risk_level).toUpperCase() as any) : "Low",
    newsSentiment: newsData.sentiment_score !== undefined ? (newsData.sentiment_score > 0.2 ? "Positive" : newsData.sentiment_score < -0.2 ? "Negative" : "Neutral") : "Neutral",
  ...
  // ─────────────────────────────────────────────────────────────
  // 1. MORNING PLAN VIEW MODEL
  // ─────────────────────────────────────────────────────────────
  const carryForwardLevel = pivotLevel ? fmtInt(pivotLevel) : "24,240";
  const openRangeEst = immediateSupport && immediateResistance
    ? `${fmtInt(immediateSupport)} – ${fmtInt(immediateResistance)}`
    : "24,210 – 24,300";
  ...
  // ─────────────────────────────────────────────────────────────
  // 2. LIVE GUIDE VIEW MODEL
  // ─────────────────────────────────────────────────────────────
  const liveActionStatus = bias === "BULLISH" && vwap && spotPrice && spotPrice >= vwap
    ? "WAIT"
    : bias === "BEARISH"

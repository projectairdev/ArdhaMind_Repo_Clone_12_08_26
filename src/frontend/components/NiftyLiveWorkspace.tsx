...
function SpotSummary() {
  ...
  return <div className={`overflow-hidden rounded-lg border border-[var(--air-line-strong)] bg-[var(--air-surface)] p-4 ${flash}`}>
    <div className="flex items-start justify-between gap-4 border-b border-slate-800 pb-3">
      ...
      <div className="air-data text-right"><div className="text-3xl font-bold tracking-tight text-white sm:text-4xl">{spot != null ? formatNumber(spot, 2) : "--"}</div><div className="text-xs text-slate-400">{hasComparison ? `${change >= 0 ? "+" : ""}${formatNumber(change, 2)} pts` : "Previous-session comparison unavailable"}</div></div>
    </div>

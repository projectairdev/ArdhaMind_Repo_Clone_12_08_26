...
export function PreMarketPlannerWorkspace() {
  ...
  return <div className="space-y-5">
    <div className="text-left"><div className="text-[10px] font-black uppercase tracking-[0.22em] text-cyan-400">Next session</div><h2 className="mt-1 text-xl font-bold text-white">Pre-Market Planner</h2><p className="mt-1 text-xs text-neutral-500">Prepare for the next NIFTY session.</p></div>
    ...
    {tab === "Evening Outlook" && <div data-premarket-panel="evening" className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2"><Card title="Overnight Risk Factors">{sinceClose.filter(item => ["Geopolitics", "Crude", "Currency/Yields", "Global Markets"].includes(safeString(item.category))).slice(0, 4).map(item => safeString(item.headline)).join(" · ") || "No verified overnight risk story is available."}</Card><Card title="Key Factors to Watch" tone="warning">{missing.length ? missing.join(", ") : "No setup gaps reported."} · Opening gap remains unknown until a genuine pre-open observation exists.</Card></div>
      <Card title="Overnight Risk Events">{overnightClusters.length ? <div className="overflow-x-auto"><div className="min-w-[42rem]"><div className="grid grid-cols-[5rem_7rem_1fr_10rem_8rem] gap-2 border-b border-slate-800 pb-1 text-[8px] font-semibold text-slate-600"><span>SEVERITY</span><span>CATEGORY</span><span>DEVELOPMENT</span><span>TRANSMISSION</span><span>TIME</span></div>{overnightClusters.map(cluster => <div key={cluster.event_cluster_id} data-overnight-cluster={cluster.event_cluster_id} className="grid grid-cols-[5rem_7rem_1fr_10rem_8rem] items-start gap-2 border-b border-slate-900 py-2 text-[9px] last:border-0"><span className={safeString(cluster.impact_level).toUpperCase()==="HIGH"?"text-rose-300":"text-amber-300"}>{safeString(cluster.impact_level)}</span><span className="text-slate-400">{safeString(cluster.category)}</span><span className="font-medium text-slate-200">{safeString(cluster.canonical_headline)}</span><span className="text-cyan-300">{safeArray(cluster.affected_channels).join(", ") || "UNAVAILABLE"}</span><span className="air-data text-slate-500">{formatDate(cluster.last_updated)}</span></div>)}</div></div> : "No meaningful verified event cluster has updated since the previous Indian market close."}</Card>
    </div>}
    ...
    {tab === "8:50 AM Briefing" && <div data-premarket-panel="briefing-850" className="space-y-4">
      ...
      <Card title="Today's Key Scheduled Events">{highImpactToday.length ? <div className="space-y-3">{highImpactToday.map(event => <div key={event.event_id} data-premarket-economic-event={event.event_id} className="border-b border-slate-800 pb-2 last:border-0"><div className="font-bold text-white">{formatDateTimeIST(event.scheduled_at_ist || event.scheduled_at)} · {safeString(event.country)} · {safeString(event.event_name)}</div><div className="mt-1 font-mono text-[10px]">{safeString(event.impact_level)} · Forecast: {event.forecast ?? "UNAVAILABLE"}{event.forecast != null ? safeString(event.unit) : ""} · Previous: {event.previous ?? "UNAVAILABLE"}{event.previous != null ? safeString(event.unit) : ""} · NIFTY {formatNumber(event.nifty_relevance, 1)}/10</div><div className="mt-1 text-cyan-200">Channels: {safeArray(event.affected_channels).join(", ") || "UNAVAILABLE"}</div></div>)}</div> : "No HIGH/CRITICAL economic event is scheduled today."}</Card>
      <div className="grid gap-4 md:grid-cols-2"><Card title="Next Scheduled Event">{nextHigh ? `${safeString(nextHigh.country)} · ${safeString(nextHigh.event_name)} · ${formatDateTimeIST(nextHigh.scheduled_at_ist || nextHigh.scheduled_at)}` : "NONE"}</Card><Card title="Corporate Announcements & Events">{corporate.length ? corporate.slice(0, 5).map(event => `${safeString(event.symbol)} · ${safeString(event.event_category)}`).join(" · ") : "UNAVAILABLE — no corporate events returned."}</Card></div>
      <Card title="Expected Opening Gap" tone={openingGap.status === "READY" ? "default" : "warning"}>{openingGap.status === "READY" ? `${safeString(openingGap.classification)} · ${Number(openingGap.gap_points) >= 0 ? "+" : ""}${formatNumber(openingGap.gap_points, 2)} points (${Number(openingGap.gap_pct) >= 0 ? "+" : ""}${formatNumber(openingGap.gap_pct, 2)}%). ${safeString(openingGap.disclaimer)}` : "UNAVAILABLE until eligible GIFT Nifty and reference observations are present."}</Card>
    </div>}

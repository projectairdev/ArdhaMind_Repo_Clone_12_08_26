...
export function UnifiedIntelligencePanel({ workspace }: { workspace: IntelligenceWorkspace }) {
  ...
  return <section data-unified-intelligence={workspace} data-intelligence-engine={intelligence.engine} className="rounded-xl border border-cyan-900/50 bg-slate-950/70 p-5 text-left space-y-4">
    ...
    <div className="grid gap-2 sm:grid-cols-3 lg:grid-cols-5">{signals.map(([name, signal]) => <div key={name} className={`rounded border p-2 ${signal.eligible ? "border-slate-800 bg-slate-900/70" : "border-slate-900 bg-slate-950"}`}><div className="flex items-center justify-between gap-1"><div className="text-[9px] font-bold uppercase text-slate-500">{name}</div><SemanticBadge value={signal.state}/></div><div className="mt-2 text-[9px] text-slate-500">{signal.eligible ? safeString(signal.evidence?.[0]) : safeString(signal.ineligibility_reason)}</div><div className="mt-2"><ProvenanceLine source={signal.source} observedAt={signal.observed_at} freshness={signal.freshness}/></div></div>)}</div>

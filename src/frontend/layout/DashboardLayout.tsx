...
import { Activity, Bell, BookOpen, Calendar, Menu, Newspaper, Radio, Settings, Sliders, X } from "lucide-react";
...
function marketLabel(value: string) { return ({ PRE_OPEN: "PRE-OPEN", OPEN: "MARKET OPEN", CLOSED: "MARKET CLOSED", HOLIDAY: "TRADING HOLIDAY" } as Record<string,string>)[value] || "INITIALIZING"; }
...
export function DashboardLayout() {
  ...
  const [mobile, setMobile] = useState(false); const [notifications, setNotifications] = useState(false); const [clock, setClock] = useState("");
  ...
  const marketStatus=marketLabel(workspaceContext.marketState); const kite=workspaceContext.brokerState === "TOKEN_EXPIRED" ? "Kite Session Expired" : workspaceContext.brokerState === "CONNECTED" ? "Kite Connected" : "Kite Disconnected";
  const feed=workspaceContext.marketState === "CLOSED" ? "Market Closed" : connectionState === "CONNECTED" && marketContext.feed_health === "HEALTHY" ? "Live" : connectionState === "CONNECTED" && marketContext.feed_health === "DEGRADED" ? "Delayed" : connectionState === "DISCONNECTED" ? "Lost" : "Initializing";
  return <div id="dashboard-layout" className={`${themeClasses.bg} ${themeClasses.text} ${fontClasses.base} flex h-screen flex-col overflow-hidden`}>
    <header className="flex min-h-16 items-center gap-3 border-b border-neutral-800 bg-neutral-950/90 px-4"><button className="lg:hidden" onClick={()=>setMobile(!mobile)}>{mobile?<X/>:<Menu/>}</button><div className="mr-auto"><div className="font-black text-white">AIR <span className={accentClasses.text}>ArdhaMind</span></div><div className="text-[9px] uppercase tracking-widest text-neutral-500">Live intelligence · Read only</div></div><div className="hidden text-xs text-neutral-300 md:block">{marketStatus} · {clock}</div><div className="hidden text-xs text-neutral-400 lg:block">{kite}</div><div className="hidden text-xs text-neutral-400 lg:block">Market Feed · {feed}</div><button aria-label="Notifications" onClick={()=>setNotifications(!notifications)} className="rounded border border-neutral-800 p-2"><Bell size={15}/></button><button aria-label="Settings" onClick={()=>navigate("settings")} className="rounded border border-neutral-800 p-2"><Settings size={15}/></button></header>
    ...
    {notifications && <div className="absolute right-16 top-16 z-50 w-72 rounded-lg border border-neutral-800 bg-neutral-950 p-4 text-xs text-neutral-400 shadow-xl">No unread intelligence alerts.</div>}

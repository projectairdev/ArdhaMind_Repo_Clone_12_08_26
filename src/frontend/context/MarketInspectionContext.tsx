...
function inspectionModel(selection: InspectionSelection, state: any, marketContext: any): InspectionModel {
  ...
  if (selection.domain === "membership") {
    ...
    return { source: membership, rows: [["Official membership", membership.membership_count === 50 ? "Verified" : "Unavailable"], ["Official members", displayValue(membership.membership_count)], ["Kite instruments resolved", membership.resolved_count == null ? "Unavailable" : `${membership.resolved_count} / ${membership.expected_count}`], ["Constituent quotes observed", breadth.coverage?.valid == null ? "Unavailable" : `${breadth.coverage.valid} / ${breadth.coverage.expected}`], ["Membership source", displayValue(membership.membership_source)], ["Source attribution", displayValue(membership.source_attribution)], ["Breadth freshness", displayValue(breadth.freshness)], ["Breadth source", displayValue(breadth.source)]], meta: { source: membership.membership_source, observed_at: membership.retrieved_at ?? breadth.timestamp, freshness: breadth.freshness } };
  }
  ...
  return { source: quote, rows: [["Instrument", selection.title], ["Latest eligible value", displayValue(quote.value ?? quote.price)], ["Absolute change", displayValue(quote.change)], ["Change %", quote.changePct == null && quote.change_pct == null ? "Unavailable" : `${Number(quote.changePct ?? quote.change_pct) > 0 ? "+" : ""}${formatNumber(quote.changePct ?? quote.change_pct, 2)}%`], ["Direction", (quote.changePct ?? quote.change_pct) == null ? "Unavailable" : Number(quote.changePct ?? quote.change_pct) > 0 ? "Positive" : Number(quote.changePct ?? quote.change_pct) < 0 ? "Negative" : "Flat"], ["Market session", displayValue(quote.sessionContext ?? quote.source_session)], ["NIFTY relevance", globalEvidence ?? "No additional deterministic interpretation available"]], meta: quote };
}

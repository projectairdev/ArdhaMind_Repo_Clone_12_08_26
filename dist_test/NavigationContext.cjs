...
var NavigationContext_exports = {};
__export(NavigationContext_exports, {
  NavigationProvider: () => NavigationProvider,
  normalizeModuleId: () => normalizeModuleId,
  useNavigation: () => useNavigation
});
module.exports = __toCommonJS(NavigationContext_exports);
var import_react = __toESM(require_react(), 1);
var import_jsx_runtime = __toESM(require_jsx_runtime(), 1);
function normalizeModuleId(raw) {
  if (!raw) return "market";
  const clean = String(raw).trim();
  if (clean === "intelligence" || clean === "market_insights" || clean === "pre_market_briefing" || clean === "market_intelligence_v2" || clean === "market_intelligence") {
  ...
  return allowed.includes(clean) ? clean : "market";
}
var NavigationContext = (0, import_react.createContext)(void 0);
function NavigationProvider({ children }) {
  const [activeModule, setActiveModuleState] = (0, import_react.useState)(() => {
    const saved = localStorage.getItem("active_module");
    return normalizeModuleId(saved);
  });
  const [marketSubTab, setMarketSubTabState] = (0, import_react.useState)(() => {
    const saved = localStorage.getItem("active_market_tab");
    if (saved === "metrics" || saved === "options") return saved;
    return "nifty";
  });
  const [newsSubTab, setNewsSubTabState] = (0, import_react.useState)("live_news");
  const [settingsOpen, setSettingsOpenState] = (0, import_react.useState)(() => {
    const saved = localStorage.getItem("active_module");
    return saved === "settings" || saved === "ardha_performance";
  });
  const [settingsSubTab, setSettingsSubTabState] = (0, import_react.useState)(() => {
    const saved = localStorage.getItem("active_module");
    return saved === "ardha_performance" ? "diagnostics" : "overview";
  });
  const [assistantOpen, setAssistantOpenState] = (0, import_react.useState)(false);
  const [assistantExpanded, setAssistantExpandedState] = (0, import_react.useState)(false);
  const [activeHighlightId, setActiveHighlightId] = (0, import_react.useState)(null);
  const setActiveModule = (0, import_react.useCallback)((m) => {
  ...
  const setMarketSubTab = (0, import_react.useCallback)((t) => {
    setSettingsOpenState(false);
  ...
  const setNewsSubTab = (0, import_react.useCallback)((t) => {
    setSettingsOpenState(false);
    setActiveModuleState("news");
    setNewsSubTabState(t);
    localStorage.setItem("active_module", "news");
  }, []);
  const setSettingsOpen = (0, import_react.useCallback)((open) => {
    setSettingsOpenState(open);
    if (open) {
      localStorage.setItem("active_module", "settings");
    }
  }, []);
  const setSettingsSubTab = (0, import_react.useCallback)((t) => {
    setSettingsOpenState(true);
    setSettingsSubTabState(t);
  }, []);
  const navigateTo = (0, import_react.useCallback)((target) => {
  ...
  const setAssistantOpen = (0, import_react.useCallback)((open) => {
    setAssistantOpenState(open);
    if (!open) {
      setAssistantExpandedState(false);
    }
  }, []);
  const toggleAssistant = (0, import_react.useCallback)(() => {
    setAssistantOpenState((prev) => {
      if (prev) setAssistantExpandedState(false);
      return !prev;
    });
  }, []);
  const setAssistantExpanded = (0, import_react.useCallback)((expanded) => {
    setAssistantExpandedState(expanded);
  }, []);
  const toggleAssistantExpanded = (0, import_react.useCallback)(() => {
...
function useNavigation() {
  const ctx = (0, import_react.useContext)(NavigationContext);
  if (!ctx) {
